...
class HeteroGATEncoder(nn.Module):
    ...
    def forward(
        ...
        h_dict = {node_type: self.dropout_layer(h) for node_type, h in h_dict.items()}
        ...
        for i, conv in enumerate(self.convs):
            ...
            h_dict_new = {node_type: F.relu(h) for node_type, h in h_dict_new.items()}

            h_dict_new = {
                node_type: self.dropout_layer(h) for node_type, h in h_dict_new.items()
            }
...
class ModalityPooling(nn.Module):
    ...
    def forward(
        ...
        # Pool mRNA
        modality_vectors["mRNA"] = self._pool(
            mrna_embeddings, batch_dict.get("gene") if batch_dict else None, "gene"
        )

        # Pool CNV
        modality_vectors["CNV"] = self._pool(
            cnv_embeddings, batch_dict.get("gene") if batch_dict else None, "gene"
        )

        # Pool DNAmeth from CpG nodes
        modality_vectors["DNAmeth"] = self._pool(
            h_dict["cpg"], batch_dict.get("cpg") if batch_dict else None, "cpg"
        )

        # Pool miRNA
        modality_vectors["miRNA"] = self._pool(
            h_dict["mirna"], batch_dict.get("mirna") if batch_dict else None, "mirna"
        )

        return modality_vectors

    def _pool(
        self, embeddings: torch.Tensor, batch: Optional[torch.Tensor], node_type: str
    ) -> torch.Tensor:
